...
/* Metodo para conexiones a API, sin token. 
por ejemplo el login
  */
export const AxiosData = async (
  endpoint, //Empoint AOU
  method, //Tipo de Peticion  POST,GET,DELETE, PUT
  body = null, //Cuerpo que puede llevar la peticion
  queryString = null, //Parametros de liga que puede llevar la peticion en la URL
) => {
...
/* Metodo para conexiones a API,por medio de token
 */
...
/* metodo llamada por metodos los cuales
retornar que el token ya no es valido por
lo tanto son regresados a login
  */
...
export const AxiosDataDIT = async (
  endpoint, //Empoint AOU
  method, //Tipo de Peticion  POST,GET,DELETE, PUT
  body = null, //Cuerpo que puede llevar la peticion
  queryString = null, //Parametros de liga que puede llevar la peticion en la URL
  ...
  switch (method) {
    ...
    case 'get':
      try {           
        ...
        AxiosResponse.data = Data;
        AxiosResponse.newId = IdNuevo;
        AxiosResponse.message = Mensaje;
        AxiosResponse.idTypeMessage = MensajeTipoId;
        AxiosResponse.stackTrace = RastreoDePila;
        AxiosResponse.typeMessage = MensajeTipo;
        AxiosResponse.message = Mensaje;
        AxiosResponse.success = Exito;

        AxiosResponse.respuestaStatus =
          Exito == 1 ? TypeService.AxiosApiOk : TypeService.AxiosApiOkFail
        return AxiosResponse
      } catch (error) {
        if (error.response) {
          if (error.response.status === 401) {
